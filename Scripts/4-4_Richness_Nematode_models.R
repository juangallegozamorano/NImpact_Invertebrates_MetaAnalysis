...
Nema$logDuration_Years <- log10(Nema$Duration_Years)
Nema$logCEC <- log10(Nema$CEC)

# Feeding guilds
table(Nema$Feeding_Guild) # All Unkown so not possible to analyze

# Habitat
table(Nema$Land_Use_Described) # Very low numbers so not possible to analyze

# load Fixed effects
FixedEffects <- readRDS('Scripts/FixedEffects_Nematodes_Richness.rds')
...
modelsNemaRich <- vector(mode = "list", length = length(FixedEffects))
...
for(i in 1:length(modelsNemaRich)){
  modeli <- modelsNemaRich[[i]]
  if(is.null(modeli)){
    I2df[i,]$ModelNr <- i
...
write.csv(I2df, file = "./Results/Nematode/Richness_model_selection_42.csv", row.names = F)

# Save models
save(modelsNemaRich, file = "./Results/Nematode/Richness_modelsNema_42.RData")
...
resid <- residuals(modeli) 
fitted <- fitted(modeli) 
...
cooks <- cooks.distance.rma.mv(modeli, parallel = "multicore", ncpus = 8)
toc()
